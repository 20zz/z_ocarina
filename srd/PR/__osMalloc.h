/*
 * $Id: __osMalloc.h,v 1.1.1.1 2003/01/17 21:06:42 tong Exp $
 *
 * ヒープ管理クラス(malloc)
 *
 * $Log: __osMalloc.h,v $
 ...
 *
 * Revision 2.7  2001-05-07 11:19:56+09  hayakawa
 * 条件コンパイルを修正
 *
...
#include <ultra64.h>
#include <stddef.h>
#include "size_t.h"             /* size_t */


#if !defined(FINAL_ROM) && DEBUG
#define OSMALLOC_DEBUG
#define OSMALLOC_DEBUG_STRUCT
#define OSMALLOC_DEBUG_FILL
#endif


/*
 * メモリーブロック構造体定義
 * OSMALLOC_DEBUG_STRUCTが定義されていないときは sizeof(__OSMemBlock) == 16bytes
 * OSMALLOC_DEBUG_STRUCTが定義されているときは   sizeof(__OSMemBlock) == 48bytes
 * となります。
...
typedef struct __OSMemBlock_s {
...
#if defined(OSMALLOC_DEBUG_STRUCT)
    ...
    u32 	__dummy[2];
#endif /* if defined(OSMALLOC_DEBUG_STRUCT) */
} __OSMemBlock;
...
#if defined(OSMALLOC_DEBUG_FILL)
...
#endif /* defined(OSMALLOC_DEBUG_FILL) */
